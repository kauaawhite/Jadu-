...
// V16a FIX — Part 2 of 4
// Paste Part 2 next (immediately after Part 1)
  ...
  for (let i = 0; i < userIds.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async uid => {
      try {
        ...
        failed++;
        console.error("batch send failed", uid, e && e.message ? e.message : e);
      }
    }));
...
function startSchedule() {
  setInterval(async () => {
    try {
      ...
      console.error("schedule error", e && e.message ? e.message : e);
    }
...
// ---------------- auto backups ----------------
setInterval(() => {
  try { backupDB(); } catch (e) { console.error("backup err", e && e.message ? e.message : e); }
}, AUTO_BACKUP_INTERVAL_MS);
// V16a FIX — Part 3 of 4
// Paste Part 3 next

// ----------------- Express Dashboard (Glass Premium UI) -----------------
...
// V16a FIX — Part 4 of 4
// Paste Part 4 last (end of file)
...
bot.onText(/\/start/, (msg) => {
  ...
  const text = `👋 Hello *${user}*!\n\nSend your *${BRAND_NAME} API Key* from the Dashboard: ${dashboardLink} (use /api YOUR_API)\n\nOnce your API key is set, just send any link — I will shorten it instantly 🔗🚀`;
...
bot.onText(/\/admin (.+)/, (msg, match) => {
  ...
  if (Number(chatId) !== Number(ALLOWED_ADMIN_ID)) return; // silent for others
...
bot.on("message", async (msg) => {
  ...
  let out = buildShortenMessage(pairs);
  if (headerFooterEnabled) {
    const db2 = readDB();
    const hdr = db2.headerText || "";
    const ftr = db2.footerText || "";
    out = `${hdr}\n\n${out}\n\n${ftr}`;
  }
...
// start express (only once) - KEEP THIS ONE
app.listen(PORT, () => console.log(`${BRAND_NAME} dashboard running on port ${PORT}`));

// graceful shutdown
process.on("SIGINT", () => { try { backupDB(); } catch (e){} process.exit(); });
process.on("SIGTERM", () => { try { backupDB(); } catch (e){} process.exit(); });
